...
void parseTxt(string &txt, vector<Mat> &imgs, vector<Mat> &gt_shapes, vector<BBox> &bboxes) {
	...
	fscanf(fd, "%d", &N);  //N images
	imgs.resize(N);
	gt_shapes.resize(N);  //68 x 2
	bboxes.resize(N);
	char img_path[256];
	double bbox[4];
	...
	for (int i = 0; i < N; i++) {
		...
		for (int j = 0; j < 4; j++) {  //x, y, width, height
			fscanf(fd, "%lf", &bbox[j]);
		}
		for (int j = 0; j < landmark_n; j++) {  //68 landmarks, pair by pair
		...
		x_min = max(0., x_min - bbox[2] / 2);  //x_min may be out of bbox(haar, may miss)
		...
		bboxes[i] = BBox(bbox[0] - x_, bbox[1] - y_, bbox[2], bbox[3]);  //bboxes may not same
		gt_shapes[i] = Mat::zeros(landmark_n, 2, CV_64FC1);
		for (int j = 0; j < landmark_n; j++) {
			gt_shapes[i].at<double>(j, 0) = x[j] - x_;  //tune the landmark coordinates
			gt_shapes[i].at<double>(j, 1) = y[j] - y_;
		}
		Rect roi(x_, y_, w_, h_);
		imgs[i] = img(roi).clone();  //ok
	}
	fclose(fd);
}
...
void data_augmentation(vector<Mat> &imgs, vector<Mat> &gt_shapes, vector<BBox> &bboxes) {
	int N = imgs.size();  //imgs are rois, they are cropped
	imgs.reserve(2 * N);  //because flipped
	gt_shapes.reserve(2 * N);
	bboxes.reserve(2 * N);
	for (int i = 0; i < N; i++) {
		Mat img_flipped;
		Mat_<double> gt_shape_flipped(gt_shapes[i].size());
		flip(imgs[i], img_flipped, 1);  //fair enough
		...
		int x_b, y_b, w_b, h_b;
		x_b = w - bboxes[i].x - bboxes[i].width;  //reverse, flip, yeah, it is
		y_b = bboxes[i].y;  //it won't affect y
		w_b = bboxes[i].width;
		h_b = bboxes[i].height;
...
int train(int start_from) {
	...
	data_augmentation(imgs_, gt_shapes_, bboxes_);  //flip
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < config.initShape_n; j++) {  //get initial shapes
