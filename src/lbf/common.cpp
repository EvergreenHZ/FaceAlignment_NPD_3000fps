...
namespace lbf {
        ...
        Mat BBox::Project(const Mat &shape) const {
                ...
                for (int i = 0; i < shape.rows; i++) {  //68
                        res(i, 0) = (shape_(i, 0) - x_center) / x_scale;  //centrelize & project
        ...
        void calcSimilarityTransform(const Mat &shape1, const Mat &shape2, double &scale, Mat &rotate) {
                ...
                temp1.col(0) = shape1.col(0) - x1_center;  //centrelize
                ...
                rotate_(0, 0) = cos_theta; rotate_(0, 1) = -sin_theta;  //rotate matrix
                rotate_(1, 0) = sin_theta; rotate_(1, 1) = cos_theta;
                rotate = rotate_;
        }


        double calcVariance(const Mat &vec) {  // Var(X) = E[X^2] - E[X]^2
        ...
        double calcMeanError(vector<Mat> &gt_shapes, vector<Mat> &current_shapes) {
                int N = gt_shapes.size();  //num of training data
        ...
        //Config::Config(const char * model_path) {
        //        dataset = "../data/68";
        //        //saved_file_name = "../model/68.model";
        //        saved_file_name = model_path;
        //        stages_n = 5;
        //        tree_n = 6;
        //        tree_depth = 5;
        //        landmark_n = 68;
