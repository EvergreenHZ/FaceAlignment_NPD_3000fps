...
void RandomTree::SplitNode(vector<Mat> &imgs, vector<Mat> &current_shapes, vector<BBox> &bboxes, \
	...
	int feats_m = config.feats_m[stage];  //random select num features, not same, stages
	...
	for (int i = 0; i < N; i++) {
		...
		for (int j = 0; j < feats_m; j++) {
			...
			SIMILARITY_TRANSFORM(x1, y1, scale, rotate);  //inverse transformation
...
#pragma omp parallel for
	for (int i = 0; i < landmark_n; i++) {  //68  independently
		TIMER_BEGIN
			vector<int> root;
		for (int j = 0; j < trees_n; j++) {  //5 trees
			int start = max(0, int(floor(j*Q - j*Q*overlap_ratio)));
			int end = min(int(start + Q + 1), N);
			int L = end - start;
			root.resize(L);
			for (int k = 0; k < L; k++) root[k] = start + k;
			random_trees[i][j].Train(imgs, current_shapes, bboxes, delta_shapes, mean_shape, root, stage);  //get one random tree
		}
